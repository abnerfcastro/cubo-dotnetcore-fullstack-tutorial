...
namespace Cubo.Core.Repositories
{
    public class InMemoryBucketRepository : IBucketRepository
    {
        ...
        public async Task<IEnumerable<string>> GetNamesAsync() => await Task.FromResult(_buckets.Select(x => x.Name));
        ...
        public async Task UpdateAsync(Bucket bucket) => await Task.CompletedTask;
